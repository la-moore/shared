  ...
  <div class="relative">
    ...
    <div ref="element"
      ...
      <div class="flex truncate relative">
        ...
        <button ref="input"
                tabindex="-1"
                class="shadow-sm flex items-center w-full px-3 h-10 border border-gray-300 rounded-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5 truncate"
                :class="[
                  errorMessage ? 'border-red-400' : 'border-gray-300',
                  disabled ? 'bg-gray-200 cursor-not-allowed' : 'bg-inherit',
...
import { setup, OptionInterface } from '../setup'
...
export default defineComponent({
  ...
  computed: {
    ...
    selectedOptions() {
      ...
      return this.proxyOptions.filter((option) => {
        if (this.multiple && Array.isArray(value)) {
